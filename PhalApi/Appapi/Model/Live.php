...
class Model_Live extends PhalApi_Model_NotORM {
	...
	public function sendGift($uid,$liveuid,$stream,$giftid,$giftcount,$ispack) {
		...
		$giftinfo=DI()->notorm->gift
					->select("type,mark,giftname,gifticon,needcoin,swftype,swf,swftime,isplatgift,sticker_id,vote_ticket")
					->where('id=?',$giftid)
					->fetchOne();
		if(!$giftinfo){
			/* 礼物信息不存在 */
        ...
        //$anthor_total=$total;//原逻辑
        $anthor_total=$giftinfo['vote_ticket']*$giftcount;//主播实际收入
        /* 幸运礼物分成 */
        if($giftinfo['type']==0 && $giftinfo['mark']==3){ //幸运礼物
            ...
            if($jackpotset['switch']=='1'){
            	$anthor_total=floor($anthor_total*$jackpotset['luck_anchor']*0.01);
            		...
            		//处理幸运礼物一级,二级分销奖励
                setAgentLevel2Profit($uid,$total);
