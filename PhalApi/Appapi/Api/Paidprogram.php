...
class Api_Paidprogram extends PhalApi_Api {
	...
	*/
    ...
    public function getPayOrder(){
        ...
        $time=checkNull($this->time);
        $sign=checkNull($this->sign);
        // var_dump($uid);var_dump($token);var_dump($object_id);var_dump($time);var_dump($sign);die;
        if($uid<0||$token==''||$object_id<1||!$time||!$sign){
            $rs['code']=1001;
            $rs['msg']='参数错误';
            return $rs;
        ...
        $info = [];
        $info['fee'] = $paidprogram_info['money']*100;
        $info['outTradeNo'] = $orderid;
        $info['pay_type'] = 'adapayJsXcx';
        $info['callback_url'] = $configpub['site'].'/Appapi/Paidprogrampay/notify_pay';
        $info['appid'] = '1021156';
        $info['key'] = 'ZtjdPau5UbRPzbxMyW2aX13wf49kOblE';
        $rs['info']= $info;
        return $rs;
    }
